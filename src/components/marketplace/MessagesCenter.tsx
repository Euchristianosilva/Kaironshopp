...
import { Send, Store, ArrowLeft, MessageCircle } from "lucide-react";
import { toast } from "sonner";

type Props = {
  basePath: "/messages" | "/seller/messages";
  activeId?: string;
};

export function MessagesCenter({ basePath, activeId }: Props) {
  const { user } = useAuth();
  ...
  return (
    <div>
      <h1 className="text-2xl sm:text-3xl font-black flex items-center gap-2 mb-6">
        <MessageCircle className="h-7 w-7 text-primary" /> Central de Mensagens
      </h1>
      <div className="grid md:grid-cols-[320px_1fr] gap-0 bg-card border border-border rounded-xl overflow-hidden min-h-[600px]">
                  ...
                  <li key={c.id}>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {c.products?.title && (
                          <div className="text-[11px] text-muted-foreground truncate">{c.products.title}</div>
                        )}
        ...
        <section className={`flex flex-col ${!activeId ? "hidden md:flex" : ""}`}>
          {activeId ? (
            <ChatPane key={activeId} conversationId={activeId} onBack={() => go(undefined)} />
          ) : (
...
function ChatPane({ conversationId, onBack }: { conversationId: string; onBack: () => void }) {
  ...
  const sendMut = useMutation({
    ...
    onSuccess: (result: any) => {
      setText("");
      if (result?.message) {
        qc.setQueryData(["conversation", conversationId], (current: any) =>
          current
  ...
  if (!data) {
    return <div className="flex-1 grid place-items-center text-muted-foreground">Carregando...</div>;
  }
  ...
  return (
    <>
      <div className="p-4 border-b border-border flex items-center gap-2">
        <button className="md:hidden p-1.5 hover:bg-secondary rounded" onClick={onBack}>
